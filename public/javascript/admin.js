...
function sortTable(columnIndex) {
  ...
  rows.sort((a, b) => {
    ...
    if (sortType === 'numeric') {
      const numA = columnIndex === 2 
        ? parseFloat(cellA.replace(' kr', '')) 
        : parseFloat(cellA);
      const numB = columnIndex === 2 
        ? parseFloat(cellB.replace(' kr', '')) 
        : parseFloat(cellB);
      
      return isAscending ? numA - numB : numB - numA;
    } else {
      // Use Swedish locale for more accurate alphabetical sorting
      return isAscending
  ...
  // Mark current header and its sort direction
