...
router.get('/api/orders/user', checkAuthenticated, (req, res) => {
  ...
  db.all(sqlOrders, [userId], (err, orders) => {
    if (err) {
...
router.get('/orders/history', checkAuthenticated, (req, res) => {
  res.render('orders-history', { firstname: req.user.first_name, lastname: req.user.last_name });
});
